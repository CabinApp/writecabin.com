...
document.addEventListener('keydown', (event) => {
    ...
    if (event.key === 'Enter' && event.target instanceof Element && event.target.matches('.article-content img'))
...
function renderBlogList(posts) {
    ...
    <a class="blog-card" href="blog.html?post=${post.slug}" style="--note-tilt:${[-1.8, 1.3, -0.7, 1.9][index % 4]}deg">
      ...
      <span class="arrow" aria-hidden="true">Read blog <i>&rarr;</i></span>
    </a>
  `,
            )
            .join('');
...
function initBlogBoard() {
    ...
    const drawThread = () => {
        ...
        let route = `M 0 ${Math.max(24, first.y - 42)} C ${first.x * 0.28} ${Math.max(18, first.y - 58)}, ${first.x * 0.66} ${first.y - 12}, ${first.x} ${first.y}`;

        for (let index = 1; index < points.length; index += 1) {
            const previous = points[index - 1];
            const next = points[index];
            const direction = next.x >= previous.x ? 1 : -1;
            const bend = Math.max(70, Math.abs(next.x - previous.x) * 0.34);
    ...
    blogList.querySelectorAll('.blog-card').forEach((card) => {
        card.addEventListener('pointermove', (event) => {
            ...
            card.style.setProperty('--note-rx', `${((event.clientY - rect.top) / rect.height - 0.5) * -5}deg`);
            card.style.setProperty('--note-ry', `${((event.clientX - rect.left) / rect.width - 0.5) * 7}deg`);
        });
...
function revealArticleMount() {
    ...
    items.forEach((item, index) => {
        ...
        item.style.transitionDelay = prefersReducedMotion ? '0ms' : `${Math.min(index * 70, 560)}ms`;
    });
...
function initAboutExperience() {
    ...
    for (let strand = 0; strand < 2; strand += 1) {
        ...
        const material = new THREE.PointsMaterial({
            ...
            size: strand ? 0.135 : 0.165,
            transparent: true,
            opacity: strand ? 0.72 : 0.92,
            alphaTest: 0.08,
            depthWrite: false,
    ...
    const layout = () => {
        ...
        anchor.position.x = compact ? 0.72 : 2.25;
        anchor.scale.setScalar(compact ? 0.78 : 1);
    ...
    const render = (time = 0) => {
        ...
        drift.rotation.y = time * 0.000035 + pointerX * 0.08;
        drift.rotation.x = pointerY * 0.035;
        helix.rotation.z = Math.sin(time * 0.00012) * 0.025;
...
function initPhilosophyDeck() {
    ...
    const spacing = () => Math.min(window.innerWidth * (window.innerWidth < 760 ? 0.62 : 0.3), 350);
    const wrap = (value) => {
        const half = count / 2;
    ...
    const render = () => {
        ...
        position += (target - position) * 0.045;
        sheets.forEach((sheet, index) => {
            ...
            sheet.style.setProperty('--carousel-opacity', Math.max(0.12, 1 - distance * 0.26).toFixed(3));
            sheet.style.setProperty('--carousel-scale', Math.max(0.76, 1 - distance * 0.085).toFixed(3));
            sheet.style.zIndex = String(Math.round(100 - distance * 10));
            sheet.classList.toggle('is-front', distance < 0.5);
        });
        if (Math.abs(target - position) > 0.002) {
    ...
    carousel.addEventListener('pointermove', (event) => {
        ...
        updateCenterHover();
        if (!centerHovered && !frame && Math.abs(target - position) > 0.002) {
            frame = requestAnimationFrame(render);
        }
    ...
    carousel.addEventListener('pointerleave', () => {
        pointerInside = false;
        updateCenterHover();
        if (!frame && Math.abs(target - position) > 0.002) frame = requestAnimationFrame(render);
    });
...
function updateWorkspaceProgress(progress) {
    const stage = progress < 0.24 ? 0 : progress < 0.5 ? 1 : progress < 0.76 ? 2 : 3;
    ...
    const draw = () => {
        ...
        const points = cards.map((card) => {
            ...
            return {
                ...
                status: card.classList.contains('done')
                    ? 'done'
        ...
        const route = window.d3?.line
            ...
            : (routePoints) =>
                  routePoints.map((point, index) => `${index ? 'L' : 'M'} ${point.x} ${point.y}`).join(' ');
